...
export default class extends Controller {
  static targets = ["musicUrl", "historyList", "volume", "volLabel"];
  ...
  connect() {
    ...
    if (this.hasVolumeTarget) this.volumeTarget.value = v;
  ...
  _currentVolume() {
    ...
    const raw = parseFloat(this.volumeTarget.value || "1");
  ...
  volumeChanged() {
    ...
    try {
      localStorage.setItem("nap_volume", String(v));
    } catch (_) {}
    if (this.hasVolLabelTarget)
      this.volLabelTarget.textContent = `${Math.round(v * 100)}%`;
    // プレビュー中の音にも即反映
  ...
  _currentDurationMs() {
    ...
    const h = parseInt(
      document.querySelector('input[name="custom_hours"]')?.value || "0",
      10
    );
    const m = parseInt(
      document.querySelector('input[name="custom_minutes"]')?.value || "0",
      10
    );
